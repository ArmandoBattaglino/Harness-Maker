...
_Last updated: 2026-04-11 - after V17.2 Pack Contract Layer - mapped by code-mapper_

> **PROJECT STATUS: V17.2 PACK CONTRACT LAYER CLOSED**
> Pack contracts are now validated by Ajv Draft 2020-12 + semver, with explicit coverage for input/output schemas, knowledgeSources, behaviorRules, artifacts, dependencies, visibleSteps, and client hook normalization. Verification: server targeted suites 51/51 PASS; client hook suite 2/2 PASS; build 507 modules.
...
| server/stores/PackStore.js | PackStore (class) | Pack persistence layer. Manages `%APPDATA%\ClaudeCodeManager\packs\` with atomic CRUD/versioning/restore plus fixture/install/bundle helpers. Validates pack contracts through `packContracts.js`, checks linked workflow existence, and validates visibleSteps against workflow node IDs. |
| server/services/packContracts.js | validatePackDefinition, normalizePackDefinition, validatePackFixture, normalizePackFixture, normalizePackInstall, validateValueAgainstSchema, JSON_SCHEMA_DRAFT | V17 pack-domain contract helpers. Uses Ajv Draft 2020-12 + ajv-formats for schema/runtime validation, semver for `engineCompatibility`, `x-packField` metadata for supported input field types, artifact format validation, dependency/visible-step invariants, and nested runtime input validation. |
...
| client/src/hooks/usePack.js | usePack (named), usePackList (named) | Pack client data layer. Provides pack list/get/create/import plus single-pack update/delete/publish/dry-run/start helpers against `/api/v1/packs`; normalizes optional contract arrays/objects into stable defaults while surfacing server validation failures. |
