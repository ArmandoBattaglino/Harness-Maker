...
| client/src/canvas/SwarmCanvas.jsx | default SwarmCanvas | Root React Flow canvas for swarm visualization. V5 Wave 3: snapToGrid enabled (20x20 grid). V5 Wave 2: NodePalette sidebar, onDragOver/onDrop handlers for palette drag-and-drop (generates nodes via generateNodeId). V5 Wave 1: undo/redo, delete handlers with cascade, context menu, copy/paste/duplicate, keyboard shortcuts. V5 bugfix 41b9a0e: stopPropagation on node/edge context menus. (Tasks #57.1, #116, #117, #120, V5 Wave 1, V5 Wave 2, V5 Wave 3) |
