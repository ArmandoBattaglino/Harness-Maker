...
_Last updated: 2026-04-08 — after Task #406 phase 2: canonical result text fix for token-boundary spacing — mapped by code-mapper_
...
| server/services/SwarmEngine.js | SwarmEngine (class), default SwarmEngine | V3 swarm orchestrator — PTY runtime for Gemini/live terminal paths, Claude structured runtime via `stream-json`, and Codex structured runtime via `codex-sdk`; tracks per-node agent state, handoffs, blockers, tool/thinking/chat/cost WS events, and budget/history. **2026-04-08 V9.1 update:** `_spawnAgent` can now route Codex to `_spawnAgentCodexSdk`, consume normalized SDK items, serialize `spawnMode='codex-sdk'`, and apply structured stop/reset/resume semantics while retaining fast PTY fallback where the SDK path should not be used. (Tasks #46, #46.3, #62.1, #145, #154, #231, #255, #327, #359, #402, BUG-AUTO-ROUTING, DEC-014, DEC-023, DEC-024, DEC-026, DEC-027, DEC-028, DEC-029) |
...
- **Last modified:** 2026-04-08 in Task #406 phase 2 by backend-dev (isCanonical chat_message handler added — replaces agentResults + patches latest chat message with canonical text)
...
- **Complexity note (Task #406p2 — canonical result text):** The `chat_message` case now has an `if (msg.isCanonical)` branch at the top. When the server sends `isCanonical: true`, the handler: (1) calls `replaceAgentChatText(msg.nodeId, msg.text)` to overwrite the streamed text_delta accumulation in agentResults, (2) calls `updateAgentState` to set `lastChatSnippet` to the canonical text, (3) calls `patchLatestChatMessage` to update the latest structured assistant chat message text. The non-canonical (else) branch retains the original append behavior. This two-path design means the client first accumulates streamed fragments for live display, then corrects them with the canonical text once the result event arrives.
