...
| server/services/SwarmEngine.js | SwarmEngine (class), default SwarmEngine | V3 swarm orchestrator — PTY runtime for Gemini/live terminal paths, Claude structured runtime via `stream-json`, and Codex structured runtime via `codex-sdk`; tracks per-node agent state, handoffs, blockers, tool/thinking/chat/cost WS events, and budget/history. **2026-04-08 V9.1 update:** `_spawnAgent` can now route Codex to `_spawnAgentCodexSdk`, consume normalized SDK items, serialize `spawnMode='codex-sdk'`, and apply structured stop/reset/resume semantics while retaining fast PTY fallback where the SDK path should not be used. **2026-04-09 commit 5d359b4:** `_handleCodexSdkTurnCompleted` emits canonical `chat_message` with `isCanonical:true` and `spawnMode:'codex-sdk'`; `execution.chatMessages` now REPLACES prior assistant entries on canonical arrival; `_applyCodexSdkItemEvent` no longer feeds ChatExtractor (eliminates duplicate WS broadcasts); fallback path added for non-terminal turns. **Task #421:** All ChatExtractor calls updated to pass `executionId` as first arg to `resetBuffer()` and use compound key semantics. (Tasks #46, #46.3, #62.1, #145, #154, #231, #255, #327, #359, #402, #421, BUG-AUTO-ROUTING, DEC-014, DEC-023, DEC-024, DEC-026, DEC-027, DEC-028, DEC-029, 5d359b4) |
...
| server/tests/swarm-engine-codex-sdk.test.js | Vitest | server/services/SwarmEngine.js, server/services/CodexSdkAdapter.js | Focused V9.1 Codex SDK structured-runtime tests: SDK spawn path emits structured WS events, PTY is bypassed on the SDK path, and structured reset aborts the active turn cleanly |
