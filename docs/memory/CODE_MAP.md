...
| server/services/SwarmEngine.js | SwarmEngine (class), default SwarmEngine | V3 swarm orchestrator — PTY runtime for Gemini/live terminal paths, Claude structured runtime via `stream-json`, and Codex structured runtime via `codex-sdk`; tracks per-node agent state, handoffs, blockers, tool/thinking/chat/cost WS events, and budget/history. **2026-04-08 V9.1 update:** `_spawnAgent` can now route Codex to `_spawnAgentCodexSdk`, consume normalized SDK items, serialize `spawnMode='codex-sdk'`, and apply structured stop/reset/resume semantics while retaining fast PTY fallback where the SDK path should not be used. **2026-04-09 commit 5d359b4:** `_handleCodexSdkTurnCompleted` emits canonical `chat_message` with `isCanonical:true` and `spawnMode:'codex-sdk'`; `execution.chatMessages` now REPLACES prior assistant entries on canonical arrival; `_applyCodexSdkItemEvent` no longer feeds ChatExtractor (eliminates duplicate WS broadcasts); fallback path added for non-terminal turns. (Tasks #46, #46.3, #62.1, #145, #154, #231, #255, #327, #359, #402, BUG-AUTO-ROUTING, DEC-014, DEC-023, DEC-024, DEC-026, DEC-027, DEC-028, DEC-029, 5d359b4) |
...
| client/src/hooks/useSwarm.js | useSwarm (named), readStoredExecution, writeStoredExecution, clearStoredExecution (module-private) | WebSocket hook for swarm execution lifecycle: connectWs(executionId) -> /ws/swarm?executionId=X; startExecution() POSTs + connects WS; stopExecution() DELETEs + closes WS. Dispatches the Swarm WS message set into Zustand, now including provider-agnostic structured-runtime handling for both `stream-json` and `codex-sdk`. Task #408: `applyExecutionSnapshot` normalizes server flat cost fields (`totalCostUsd` etc.) into nested `totalCost` objects so cost data persists through reconciliation. (Tasks #63, #109, #114, #128, #238-#241, POST-V5 FOLLOW-UP 2, #404, #408) |
...
### `client/src/store/SwarmContext.jsx` :: `replaceNodeChatMessages(nodeId, canonicalMsg, predicate)` (NEW — commit 5d359b4)
...
- **Last modified:** 2026-04-09 in commit 5d359b4 by backend-dev/frontend-dev
...
- **Last modified:** 2026-04-09 in commit 5d359b4 by backend-dev/frontend-dev (isCanonical handler now uses replaceNodeChatMessages instead of patchLatestChatMessage — collapses all text_delta fragments into one canonical message, fixing BUG-CHAT-3)
...
- **Complexity note (Task #406p2 — canonical result text, updated 5d359b4):** The `chat_message` case has an `if (msg.isCanonical)` branch at the top. When the server sends `isCanonical: true`, the handler: (1) calls `replaceAgentChatText(msg.nodeId, msg.text)` to overwrite the streamed text_delta accumulation in agentResults, (2) calls `updateAgentState` to set `lastChatSnippet` to the canonical text, (3) calls `replaceNodeChatMessages` (changed from `patchLatestChatMessage` in 5d359b4) to REPLACE ALL matching structured assistant chat messages with a single canonical message — this eliminates the bug where earlier text_delta fragments remained in the array and ChatPanel grouping produced duplicated/truncated output (BUG-CHAT-3). The non-canonical (else) branch retains the original append behavior. This two-path design means the client first accumulates streamed fragments for live display, then collapses them with one canonical message once the result event arrives.
