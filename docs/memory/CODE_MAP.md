...
| server/services/SwarmEngine.js | SwarmEngine (class), default SwarmEngine | V3 swarm orchestrator — PTY runtime for Gemini/live terminal paths, Claude structured runtime via `stream-json`, and Codex structured runtime via `codex-sdk`; tracks per-node agent state, handoffs, blockers, tool/thinking/chat/cost WS events, and budget/history. **2026-04-08 V9.1 update:** `_spawnAgent` can now route Codex to `_spawnAgentCodexSdk`, consume normalized SDK items, serialize `spawnMode='codex-sdk'`, and apply structured stop/reset/resume semantics while retaining fast PTY fallback where the SDK path should not be used. **2026-04-09 commit 5d359b4:** `_handleCodexSdkTurnCompleted` emits canonical `chat_message` with `isCanonical:true` and `spawnMode:'codex-sdk'`; `execution.chatMessages` now REPLACES prior assistant entries on canonical arrival; `_applyCodexSdkItemEvent` no longer feeds ChatExtractor (eliminates duplicate WS broadcasts); fallback path added for non-terminal turns. **2026-04-09 V10.4:** structured chat now carries stable per-turn `turnId`; canonical replacement/persistence is scoped to `nodeId + turnId`, so repeated same-node runs keep one canonical assistant message per completed turn instead of overwriting node history. **Task #421:** All ChatExtractor calls updated to pass `executionId` as first arg to `resetBuffer()` and use compound key semantics. (Tasks #46, #46.3, #62.1, #145, #154, #231, #255, #327, #359, #402, #421, #467, BUG-AUTO-ROUTING, DEC-014, DEC-023, DEC-024, DEC-026, DEC-027, DEC-028, DEC-029, 5d359b4) |
...
| server/routes/swarm.js | swarmRoutes (factory fn), resolveBroadcastNodeTargets, serializeSessionOutput | REST API for swarm execution control: start, pause, resume, stop, status, agent output, broadcast + execution history endpoints. Per-node structured stop/reset route now supports both `stream-json` and `codex-sdk` runtimes. Factory pattern: accepts swarmEngine + sessionManager + scaffoldProviders. (Tasks #47.1 + #59 + #112 + V5-W4 + #402) |
