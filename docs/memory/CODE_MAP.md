...
- **Complexity note (Tasks #105-#107, #112):** Stop button condition: `executionStatus === 'running' || executionStatus === 'paused'`. Run button: always rendered when `executionStatus === 'idle'` (Task #112 — was conditionally hidden); disabled with `disabled` attr + `title` tooltip when workflowDef or activeProjectId is missing. Previously used conditional rendering (`&&`) which caused the button to disappear entirely when preconditions were not met. Task #112 changed to always render + disable so users see the button and can understand why it cannot be clicked via the tooltip.
- **Last modified:** 2026-03-31 in Task #112 by orchestrator (Run button: conditional rendering → always-rendered disabled+tooltip pattern when idle; title tooltip: 'Select a project first' | 'Generate a workflow below first' | 'Run workflow')
