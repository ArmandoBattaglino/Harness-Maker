...
| client/src/store/SwarmContext.jsx | useSwarmStore (default + named) | Zustand v4 store for V3 swarm execution state. Holds agentStates, edgeCounters, budget, inboxItems, interAgentFeed, departmentStack breadcrumb, selectedNodeId, ptyExplosionNodeId, wsConnected, workflowDef. Structured runtime agent states now truthfully model `spawnMode='stream-json' | 'codex-sdk' | 'pty'`. V5 Wave 4: updateAgentState now auto-tracks timestamps (started/done/error) on status transitions. (Tasks #52, #117, V5-W4, #404) |
| client/src/canvas/nodes/AgentNode.jsx | default AgentNode | React Flow custom node type="agent". Subscribes to useSwarmStore(agentStates[id]). 6 visual statuses (idle/running/done/paused/error/preview), target Handle top + source Handle bottom, lastOutputSnippet scrollable bg-black/40 container (last 4 lines, green monospace pre, blinking cursor when running), handoffCount badge, validation warning badge (amber ! when empty systemPrompt - FR-V5-45). POST-V5 FOLLOW-UP 2: drop-preview mode renders a dashed ghost card, disables pointer events, hides handles/warnings/snippet/handoff badges, and shows a "Drop preview" status during palette drags. (Tasks #53.1, #65, V5 Wave 3, POST-V5 FOLLOW-UP 2) |
...
| client/src/hooks/useSwarm.js | useSwarm (named), readStoredExecution, writeStoredExecution, clearStoredExecution (module-private) | WebSocket hook for swarm execution lifecycle: connectWs(executionId) -> /ws/swarm?executionId=X; startExecution() POSTs + connects WS; stopExecution() DELETEs + closes WS. Dispatches the Swarm WS message set into Zustand, now including provider-agnostic structured-runtime handling for both `stream-json` and `codex-sdk`. restorePersistedExecution uses raw fetch for hydration with 404 -> clearStoredExecution + clearExecutionState (BUG-SWARM-UI-2 fix, Tasks #238-#241). agentStates top-level subscription removed (Task #109); uses useSwarmStore.getState() inside handler. Cleanup useEffect depends on [workflowId] (Task #114 - BUG-TOOLBAR-2). updateTriggerState selector added + 3 trigger event cases (Task #128 - BUG-TRIGGER-1). POST-V5 FOLLOW-UP 2 reordered internal callbacks so applyExecutionSnapshot is initialized before reconcileClosedExecution, fixing the SwarmView mount/hydration crash. (Tasks #63, #109, #114, #128, #238-#241, POST-V5 FOLLOW-UP 2, #404) |
...
- **Purpose:** Custom React Flow node for visualizing a single swarm agent. Reads live state from useSwarmStore to render status color, output snippet, and handoff count. V5 Wave 3: added validation warning badge - amber circle with "!" at top-right when data.systemPrompt is empty/whitespace (FR-V5-45). POST-V5 FOLLOW-UP 2: when `data.isDropPreview` is truthy, renders a ghost "preview" variant for live palette drags instead of a fully interactive runtime node. Registered as nodeTypes["agent"] in the React Flow canvas.
- **Called by:** SwarmCanvas.jsx — registered in module-level nodeTypes map as nodeTypes.agent (Task #57.1)
- **Calls:** useSwarmStore (selector: s.agentStates[id]), Handle (from @xyflow/react), Position (from @xyflow/react), stripAnsi (utils/stripAnsi.js)
...
- **Output:** JSX - bordered card with status color, agent icon, label, status text, optional validation badge (amber ! when empty systemPrompt), optional lastOutputSnippet, optional handoffCount badge. Preview mode shows status "Drop preview", disables pointer events, and omits both handles plus runtime-only badges/snippet blocks.
- **Side effects:** none (read-only store subscription + pure render)
- **Complexity note:** statusColors lookup uses `statusColors[status] || statusColors.idle` - unknown status values fall back to idle styling rather than throwing. `lastOutputSnippet.split('\n').slice(-4).join('\n')` trims to last 4 lines of output for the micro-log display (changed from 3 in Task #65). The output area is a scrollable bg-black/40 container (max-h-16, overflow-y-auto) with a green monospace pre. A blinking cursor (`<span className="animate-pulse">`) is appended only when status === 'running'. Validation badge: `!data?.systemPrompt?.trim()` renders absolute-positioned amber circle with "!" and title="Empty system prompt". Preview mode forces `status = 'preview'`, adds ghost styling, and short-circuits handles/warning badges/unviewed markers/snippet/handoff count so the temporary drag node stays visually lightweight and non-interactive.
- **Last modified:** 2026-04-07 in POST-V5 FOLLOW-UP 2 by frontend-dev (drop-preview ghost state added for Agent palette drags)
...
- **Last modified:** 2026-04-07 in POST-V5 FOLLOW-UP 2 by frontend-dev (declaration order fixed to unblock SwarmView mount)
