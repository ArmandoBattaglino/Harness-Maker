...
| server/stores/PackStore.js | PackStore (class) | Pack persistence/distribution layer. Manages packs, versions, fixtures, installs, local bundle export/import, workflow dependency rebinding on import, and fork-to-draft provenance metadata. |
...
| server/services/PackResolver.js | PackResolver (class), PACK_ENGINE_VERSION | Resolves pack starts by loading PackDefinition + linked workflow, validating pack engine compatibility and required dependencies, producing reserved workflow context overlays, priority-sorted behavior directives, and additive pack metadata for SwarmEngine. |
| server/services/PackResultBuilder.js | buildPackResult, buildPackArtifacts, buildVisibleStepStatuses | Builds pack-shaped operator results from live or persisted workflow execution data, including declared outputs/artifacts and visible-step statuses derived from agentStates or nodeSnapshots. |
...
| server/tests/marketing-video-harness-stress.test.js | Vitest suite | Hermetic V17 stress suite for complete Marketing Video Campaign Harness pack: contract surfaces, repeated long/unicode starts, invalid input rejection, distribution roundtrip, fixture run, and publish gate. |
...
| client/src/lib/constants.js | NAV_ITEMS, STATUS_COLORS | Shared UI constants. V17.4 adds Pack Builder sidebar navigation entry. |
...
| client/src/store/SwarmContext.jsx | useSwarmStore | Zustand execution store. V17.3 adds additive `packRun`, `packResult`, and `hydratePackRuntime()` state without splitting pack runs into a separate runtime store. |
...
| client/src/views/PackLibraryView.jsx | default PackLibraryView | Operator-first pack surface. V17 marketing stress follow-up adds schema-aware generated form controls/coercion for enum, boolean, integer/number, object/array JSON, textarea, and text fields before pack start; launch polling hydrates terminal packRun/packResult. |
| client/src/views/MarketingVideoHarness.e2e.test.jsx | Vitest/RTL suite | Hermetic marketing-video operator flow: generated form fill, explicit project binding, start payload, completed status/results hydration, and final packResult assertions. |
