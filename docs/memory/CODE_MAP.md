...
_Last updated: 2026-04-11 - after V17.1 Pack Domain Foundation - mapped by code-mapper_

> **PROJECT STATUS: V17.1 PACK DOMAIN FOUNDATION CLOSED**
> The codebase now includes the first persisted pack substrate on top of the workflow-first runtime: PackStore, pack CRUD/version routes, bootstrap wiring, a minimal `usePack` hook, and additive pack metadata across swarm status/results/history. Verification for this baseline: server targeted suites 46/46 PASS; client pack-hook suite 2/2 PASS.
...
- `server/services/SwarmEngine.js` — serialized agent snapshots now expose backend-authoritative `acceptsMessages` + `messageTransport`; broadcast delivery can reuse completed PTY sessions, resume completed `stream-json` / `codex-sdk` conversations on the same session/thread, and queue operator follow-up behind soft/hard structured interrupts.
- `server/routes/swarm.js` — broadcast targeting now scopes workflow recipients first and leaves final message-delivery eligibility to `SwarmEngine.sendBroadcast()` instead of hardcoding `running + sessionId` route heuristics.
- `client/src/hooks/useSwarm.js` — terminal execution states keep or restore WS connectivity when any hydrated agent remains `acceptsMessages`, so completed-but-messageable runs stay interactive.
- `client/src/canvas/ChatPanel.jsx` — chat input availability and target selection now key off backend-authoritative `acceptsMessages`, not only active execution state.
- `client/src/views/SwarmView.jsx` — structured runtime session controls stay visible for terminal executions when any structured agent remains messageable.
...
| File | Key Exports | Purpose |
|------|-------------|---------|
| server/index.js | (main) | Full bootstrap: binary discovery, config load, middleware, WorkflowStore + PackStore startup, `/api/v1/workflows`, `/api/v1/packs`, `/api/v1/swarm`, SwarmEngine + ExecutionHistoryStore wiring, WebSocket routing, SPA fallback, and graceful shutdown. |
...
| server/services/BinaryDiscovery.js | discoverClaudeBinary, discoverCodexBinary, discoverGeminiBinary | Runtime binary discovery for Claude, Codex, and Gemini; Codex/Gemini discovered from local install/env hints while Claude keeps the 4-step lookup path |
...
| server/services/index.js | (barrel) | Re-exports ConfigStore, ProcessRegistry, binary discovery helpers, and CodexSdkAdapter helpers for clean server imports |
| server/services/CodexSdkAdapter.js | buildCodexSdkClientOptions, buildCodexSdkThreadOptions, createCodexSdkClient, getCodexSdkThread, runCodexSdkTurnStreamed, normalizeCodexSdkItem | Thin adapter around `@openai/codex-sdk`. Centralizes client/thread option shaping, start/resume thread access, streamed turn startup, and normalization of Codex SDK items into SwarmEngine-friendly structured event shapes. (Task #401) |
...
| server/stores/PackStore.js | PackStore (class) | Pack persistence layer for V17.1. Manages `%APPDATA%\ClaudeCodeManager\packs\` with atomic create/update/delete, version snapshots, restore, fixture storage, install records, and local bundle import/export helpers. Validates and normalizes packs through `packContracts.js` and checks linked workflow existence via WorkflowStore. |
| server/services/packContracts.js | validatePackDefinition, normalizePackDefinition, validatePackFixture, normalizePackFixture, normalizePackInstall, validateValueAgainstSchema | Pack-domain contract helpers for V17.1. Defines allowed lifecycle/status enums and validation rules for `runtimePolicy`, `dependencies`, `inputSchema`, `knowledgeSources`, `behaviorRules`, `outputSchema`, `artifactDefinitions`, `visibleSteps`, fixture/install records, and nested input validation. |
| server/services/PackResolver.js | PackResolver (class) | Resolves a pack start request into workflow execution inputs: explicit project binding, reserved `workflowContextPatch`, mergeStrategy-aware pack knowledge overlays, priority-sorted behavior rules, and additive `packMetadata` for SwarmEngine. |
| server/services/PackResultBuilder.js | buildPackResult, buildPackArtifacts | Builds pack-shaped operator results from the existing workflow execution envelope. Maps declared outputs/artifacts plus visible-step metadata over live or persisted swarm execution data without replacing the workflow artifact builder. |
| server/stores/ExecutionHistoryStore.js | ExecutionHistoryStore (class) | Per-workflow execution history persistence to CONFIG_DIR/execution-history/<workflowId>.json. Methods: init, addEntry, getHistory, getEntry. Max 100 entries per workflow, trims oldest on overflow. Uses write-file-atomic + path-traversal guard. V17.1 adds additive `packId`, `packVersion`, and `packRun` metadata. (V5 Wave 4, V17.1) |
...
| server/services/StreamJsonParser.js | StreamJsonParser (class), default StreamJsonParser | Stateless NDJSON line parser for Claude CLI `--output-format stream-json` output. Transforms raw JSON lines into typed application events (text_delta, tool_start/delta/stop, thinking, message_start/delta/stop, result, api_retry, etc.). Tracks active content block type for content_block_stop dispatch. Never throws — all errors return error-type events. 1 MB line cap (SEC-SJ-03). Task #406p2: `_parseResult` now extracts `resultText` from result events for canonical text replacement. (Task #357, #406p2, DEC-027, DEC-029) |
...
| server/services/ChatExtractor.js | ChatExtractor (class) | Extracts clean chat messages from agent PTY streams for the Unified Chat View. Bypassed for Claude stream-json agents; active only for PTY agents (Codex/Gemini). Internal `_buffers` Map uses compound key `${executionId}:${nodeId}` for execution-scoped buffer isolation. Methods: feed, flush, resetBuffer, cleanup, cleanupNode, systemMessage, registerNodePrompt. Task #421: refactored from nodeId-only keys to compound keys; `resetBuffer(executionId, nodeId)` now requires executionId; `cleanup(executionId)` scoped to prefix match. (Tasks #421) |
| server/services/SwarmEngine.js | SwarmEngine (class), default SwarmEngine | Existing swarm orchestrator plus V17.1 additive pack-awareness: `startExecution()` accepts `workflowContextPatch` + `packMetadata`; `_buildInitialWorkflowContext()` merges pack overlays; `_buildSystemPrompt()` renders pack knowledge and priority-sorted pack behavior directives; status/history snapshots include `packRun` metadata without changing workflow-only execution behavior. |
...
| server/routes/swarm.js | swarmRoutes (factory fn), resolveBroadcastNodeTargets, serializeSessionOutput | REST API for swarm execution control: start, pause, resume, stop, status, agent output, broadcast + execution history endpoints. V17.1 extends live/persisted results to surface additive `packRun` metadata and pack-shaped results while leaving workflow-only responses valid. |
...
| client/src/hooks/usePack.js | usePack (named), usePackList (named) | V17.1 pack client data layer. Provides pack list/get/create/import plus single-pack update/delete/publish/dry-run/start helpers against `/api/v1/packs`, intentionally kept separate from `useWorkflow.js` so pack-first surfaces do not overload workflow CRUD state. |
...
| client/src/store/SwarmContext.jsx | useSwarmStore (default + named) | Zustand v4 store for V3 swarm execution state. Holds agentStates, edgeCounters, budget, inboxItems, interAgentFeed, departmentStack breadcrumb, selectedNodeId, ptyExplosionNodeId, wsConnected, workflowDef. Structured runtime agent states now truthfully model `spawnMode='stream-json' | 'codex-sdk' | 'pty'`. V5 Wave 4: updateAgentState auto-tracks timestamps. Task #407: `setWorkflowDef` auto-clears execution state on workflow ID change via `buildClearedExecutionState()`. Task #406p2: new `replaceAgentChatText` action for canonical result text replacement. Commit 5d359b4: new `replaceNodeChatMessages` action to collapse all text_delta fragments into one canonical chat message. V10.4 documents `canonicalTurnId` on structured agent state so client guards know which turn the canonical message belongs to. (Tasks #52, #117, V5-W4, #404, #407, #406p2, #468, 5d359b4) |
| client/src/canvas/nodes/AgentNode.jsx | default AgentNode | React Flow custom node type="agent". Subscribes to useSwarmStore(agentStates[id]). 6 visual statuses (idle/running/done/paused/error/preview), target Handle top + source Handle bottom, lastOutputSnippet scrollable container, handoffCount badge, validation warning badge, cost badge (dual-format: `totalCost.costUsd` OR flat `totalCostUsd`, no longer gated on isStreamJson). POST-V5 FOLLOW-UP 2: drop-preview ghost mode. (Tasks #53.1, #65, V5 Wave 3, POST-V5 FOLLOW-UP 2, #408) |
...
| client/src/canvas/nodes/ConditionalNode.jsx | default ConditionalNode | Diamond-shaped IF/THEN router node. Amber theme. Top target handle, per-rule source handles + default handle at bottom. Rule count badge. (V5 Wave 5, FR-V5-56/59) |
| client/src/canvas/nodes/MergeNode.jsx | default MergeNode | Hexagonal merge/join node (CSS clip-path). Cyan theme. 3 left target handles, 1 right source handle. waitFor config display (all/any/count). (V5 Wave 5, FR-V5-60) |
| client/src/canvas/nodes/DelayNode.jsx | default DelayNode | Delay timer node with clock icon. Orange theme. Formatted time display (h/m/s). Top target, bottom source. (V5 Wave 5, FR-V5-64/66) |
| client/src/canvas/nodes/LoopNode.jsx | default LoopNode | Loop iteration node with circular arrow icon. Violet theme. Iteration badge, exit condition preview. Bottom "loop" source handle + right "exit" source handle. (V5 Wave 5, FR-V5-68/71) |
| client/src/canvas/nodes/ErrorHandlerNode.jsx | default ErrorHandlerNode | Red lightning bolt error handler node. Watched nodes count display. Source-only (bottom). (V5 Wave 5, FR-V5-73) |
| client/src/canvas/nodes/SubWorkflowNode.jsx | default SubWorkflowNode | Double-border nested workflow reference node. Gray theme. Top target, bottom source. Referenced workflowId display. (V5 Wave 5, FR-V5-77) |
...
| client/src/canvas/AgentInspector.jsx | default AgentInspector, AgentFields, DepartmentFields, TriggerFields, ConditionalFields, MergeFields, DelayFields, LoopFields, ErrorHandlerFields, SubWorkflowFields, ExecutionInfo, useDebouncedField, CollapsibleSection (internals) | Full edit panel for selected canvas node. Editable label, type-specific configuration sections. Live status + output from Zustand. "Open Terminal" button. V5 Wave 4: ExecutionInfo section. V5 Wave 5: 6 new field components for flow control nodes (ConditionalFields, MergeFields, DelayFields, LoopFields, ErrorHandlerFields, SubWorkflowFields). |
...
| client/src/utils/runtimeModes.js | STRUCTURED_SPAWN_MODES, isStructuredSpawnMode | Shared helper for client structured-runtime checks. Treats both `stream-json` and `codex-sdk` as structured modes so chat/tool/thinking/cost UI logic stays provider-agnostic. (Task #404) |
...
| client/src/canvas/NodePalette.jsx | default NodePalette, PaletteCard (internal), NODE_CARDS (const) | Collapsible left sidebar with draggable node type cards. V5 Wave 5: expanded from 4 to 10 entries (added Conditional Router, Merge/Join, Delay Timer, Loop, Error Handler, Sub-Workflow). Drag sets application/reactflow-type + application/reactflow-subtype on dataTransfer. (FR-V5-25 through FR-V5-29, V5 Wave 2, V5 Wave 5) |
...
| client/src/canvas/SwarmCanvas.jsx | default SwarmCanvas | Root React Flow canvas for swarm visualization. V5 Wave 5: 6 new imports + nodeTypes entries (conditional, merge, delay, loop, errorHandler, subWorkflow) + 6 onDrop data initializers. V5 Wave 3: snapToGrid (20x20). V5 Wave 2: NodePalette, onDrop. V5 Wave 1: undo/redo, context menu, keyboard shortcuts. V5 bugfix 41b9a0e: stopPropagation. POST-V5 FOLLOW-UP 2: Agent palette drags now render a live ghost node preview on the same snapped coordinates used by the final drop path. (Tasks #57.1, ..., V5 Wave 1-5, POST-V5 FOLLOW-UP 2) |
...
| client/src/hooks/useSwarm.js | useSwarm (named), readStoredExecution, writeStoredExecution, clearStoredExecution (module-private) | WebSocket hook for swarm execution lifecycle: connectWs(executionId) -> /ws/swarm?executionId=X; startExecution() POSTs + connects WS; stopExecution() DELETEs + closes WS. Dispatches the Swarm WS message set into Zustand, now including provider-agnostic structured-runtime handling for both `stream-json` and `codex-sdk`. Task #408: `applyExecutionSnapshot` normalizes server flat cost fields (`totalCostUsd` etc.) into nested `totalCost` objects so cost data persists through reconciliation. Task #423: `chat_message` handler now has `if (!msg.nodeId) break;` guard to silently drop nodeId-less messages. V10.4: same-execution snapshot hydration preserves `canonicalReceived/canonicalTurnId`, structured `chat_message` handling keys canonical replacement and fragment dropping by `turnId`, and REST hydration dedupe now treats same-node earlier turns as distinct canonical history instead of stale duplicates. (Tasks #63, #109, #114, #128, #238-#241, POST-V5 FOLLOW-UP 2, #404, #408, #423, #467, #468) |
...
| File | Key Exports | Purpose |
|------|-------------|---------|
| client/package.json | (config) | Client dependencies — now includes @xyflow/react@12.10.1 (React Flow v12 graph canvas) and zustand@4.5.7 (v4, not v5) alongside react@18.2, react-markdown, xterm, xterm-addon-fit. Task #425: added rehype-sanitize@^6.0.0 for XSS prevention in ChatMessage ReactMarkdown. (Tasks #51, #425) |
...
### `server/services/SwarmEngine.js` :: `SwarmEngine._spawnAgent(executionId, nodeId, spawnOptions)` (NEW — Task #359, MODIFIED — BUG-AUTO-ROUTING)
- **Purpose:** Dispatcher that routes agent spawn to the correct path based on provider. Claude providers go to `_spawnAgentStreamJson` (DEC-027); all others go to `_spawnAgentPty`. Resolves effective provider from spawnOptions.requestedProvider / spawnOptions.provider, then configuredModel from spawnOptions.requestedModel / node.data.model / workflow defaultModel. When effectiveProvider is still AUTO after model-based resolution, consults `execution.providerStrategy.activeProvider` — if activeProvider is 'claude', routes to stream-json; if another non-AUTO provider, routes accordingly; if null/AUTO, falls back to PTY.
- **Called by:** SwarmEngine.startExecution (line 3465), SwarmEngine._ensureAgentPty (line 4613 — handoff path), SwarmEngine._spawnChildExecution (line 5419), SwarmEngine._onDone (line 6008 — stream-json reinject path)
- **Calls:** normalizeRuntimeProvider, isSupportedRuntimeModel, SwarmEngine._spawnAgentStreamJson (Claude), SwarmEngine._spawnAgentPty (non-Claude or final AUTO fallback)
- **Inputs:** executionId (string), nodeId (string), spawnOptions (object — optional: requestedProvider, provider, requestedModel, reinjectPrompt)
- **Output:** Promise\<void\>
- **Side effects:** delegates to either _spawnAgentStreamJson or _spawnAgentPty (see their entries)
- **Complexity note:** Three-tier provider resolution: (1) explicit requestedProvider/provider → normalizeRuntimeProvider, (2) model-based: if AUTO and configuredModel exists, resolve provider from model name via isSupportedRuntimeModel, (3) strategy-based: if still AUTO, consult execution.providerStrategy.activeProvider. This ensures generated workflows with no explicit model on nodes still route Claude agents to stream-json.
- **Last modified:** 2026-04-08 in BUG-AUTO-ROUTING by debugger (added providerStrategy.activeProvider consultation when effectiveProvider remains AUTO after model resolution); previously 2026-04-08 in Task #359 by backend-dev
...
- **Last modified:** 2026-04-09 in Task #427 by backend-dev (removed ChatExtractor.feed from stream-json text_delta and message handlers); previously 2026-04-08 in Task #359 by backend-dev

### `server/services/SwarmEngine.js` :: `SwarmEngine._handleStreamJsonResult(executionId, nodeId, resultEvt, handoffTargets)` — MODIFIED Task #406p2
- **Purpose:** Handle a stream-json `result` event for a Claude agent turn. (1) Increments turnCount, (2) extracts and broadcasts cost/token usage via WS agent_cost event, (3) stores sessionId from result for future --resume turns, (4) handles error results (sets needsRepair), **(4b) if resultEvt.resultText is present, replaces `_streamJsonAccumulatedText` with canonical text, updates lastOutputSnippet, broadcasts a corrective `chat_message` with `isCanonical: true` flag, AND (Task #417) replaces execution.chatMessages entries for the nodeId with a single canonical message (filter+push+slice to 500 cap),** (5) scans accumulated text for __HANDOFF__/__DONE__ tokens using a fresh HandoffParser instance, (6) routes to _onHandoff (first handoff found) or _onDone (explicit or implicit done — DEC-029), (7) checks doNotSpawnNextTurn graceful stop flag, (8) schedules 30s post-result tree-kill timeout via setTimeout + treeKill for child process cleanup, (9) resets _streamJsonAccumulatedText for next turn.
- **Called by:** SwarmEngine._spawnAgentStreamJson (inside readline 'line' handler, on evt.type === 'result')
- **Calls:** HandoffParser (constructor + feed — for token scanning), SwarmEngine._onHandoff, SwarmEngine._onDone, SwarmEngine._broadcastAgentStatus, SwarmEngine._syncExecutionStatusFromAgents, treeKill (npm tree-kill — 30s deferred), this._wsBroadcast (agent_cost + NEW chat_message isCanonical)
- **Inputs:** executionId (string), nodeId (string), resultEvt (object — { usage: { input, output }, costUsd, durationMs, sessionId, isError, errorMessage, resultText }), handoffTargets (string[] — outgoing edge target IDs)
- **Output:** void (synchronous)
- **Side effects:** mutates state.turnCount, totalInputTokens, totalOutputTokens, totalCostUsd, streamJsonSessionId, needsRepair, _streamJsonAccumulatedText, lastOutputSnippet; broadcasts WS agent_cost event; **broadcasts WS chat_message with isCanonical:true when resultText present;** **Task #417: updates execution.chatMessages — filters out prior assistant entries for the nodeId, pushes single canonical message, slices to 500 cap (mirrors Codex SDK path pattern);** may trigger _onHandoff or _onDone (which have their own side effects); schedules 30s setTimeout for tree-kill of child process; may set status='done' if doNotSpawnNextTurn is set
- **Complexity note:** Uses a fresh HandoffParser instance to scan accumulated text (not the readline parser) — this is the same pattern as PTY path but applied to the full turn text rather than streaming chunks. Implicit __DONE__ (no token found) is treated as explicit done per DEC-029. The 30s tree-kill timeout is a safety net for processes that linger after sending the result event. **Task #406p2 note:** The canonical text replacement at step 4b fixes token-boundary spacing — Claude CLI text_delta tokens carry tokenizer whitespace (e.g. " con", " su", "ma") that produces garbled concatenation. The result event's `result` field has the correctly assembled text. **Task #417 note:** Lines ~5370-5381 now also update `execution.chatMessages` with a filter+push+slice pattern: filters out all prior assistant messages for the nodeId, pushes the canonical message, and caps at 500 entries. This ensures REST hydration (GET /status) returns clean canonical text instead of stale text_delta fragments that would re-inject garbled content on page refresh.
- **Last modified:** 2026-04-09 in Task #417 by backend-dev (added execution.chatMessages canonical replacement — filter+push+slice pattern)
...
- **Last modified:** 2026-04-08 in Task #407 by frontend-dev (BUG-DL-STALE-STATE-1: setWorkflowDef now auto-clears execution state on workflow ID change via buildClearedExecutionState; previously Task #117 BUG-SWARM-3)

### `client/src/store/SwarmContext.jsx` :: `buildClearedExecutionState()` (module-private helper — NEW Task #407)
- **Purpose:** Returns a plain object with all execution-related Zustand fields reset to their initial/empty values. Centralizes the "clear everything" shape so `clearExecutionState`, `reset`, and `setWorkflowDef` all produce identical cleared state.
- **Called by:** `setWorkflowDef` (when workflow ID changes and stale state exists), `clearExecutionState` action, `reset` action
- **Calls:** none (pure factory)
- **Inputs:** none
- **Output:** Object with 20 fields: activeExecutionId=null, executionStatus='idle', runtimeBlocker/runtimeProvider/providerStrategy/lastFallback=null, agentStates/agentResults/triggerStates/edgeCounters={}, budget={estimatedTokensUsed:0,limitTokens:0}, inboxItems/resolvedHitlIds/interAgentFeed/chatMessages=[], chatFilter='all', sidePanelMode='chat', sidePanelOpen=true, focusedDepartmentId=null, departmentStack=[], selectedNodeId=null, ptyExplosionNodeId=null, wsConnected=false
- **Side effects:** none (pure function)
- **Last modified:** 2026-04-08 in Task #407 by frontend-dev (BUG-DL-STALE-STATE-1: extracted from clearExecutionState, now also used by setWorkflowDef)
...
- **Purpose:** Store a generated workflow definition in Zustand so it persists across component unmounts (tab navigation). Added to fix BUG-SWARM-3. Task #407 (BUG-DL-STALE-STATE-1): now auto-clears all execution state via `buildClearedExecutionState()` when the workflow ID changes (or transitions from null to a new workflow while stale execution state exists), preventing previous-run node status/chat/results from bleeding into a freshly-generated workflow.
...
- **Calls:** Zustand set, buildClearedExecutionState() (when workflow ID changes and stale state exists)
- **Inputs:** def (object — `{ id, name, nodes[], edges[], ... }` — the full workflow definition returned by scaffold endpoint; or null to clear)
- **Output:** void
- **Side effects:** updates workflowDef slice in store; when workflow ID differs from current AND stale execution state exists (activeExecutionId != null OR executionStatus != 'idle' OR agentStates non-empty), replaces all execution-related fields with cleared defaults via `buildClearedExecutionState()`
- **Complexity note (Task #407 — BUG-DL-STALE-STATE-1):** The stale-state check uses 3 conditions ORed: `activeExecutionId !== null`, `executionStatus !== 'idle'`, `Object.keys(agentStates).length > 0`. Same-workflow updates (prevId === nextId) skip the clear so that in-flight execution state is preserved during save/reload of the same workflow.
- **Last modified:** 2026-04-08 in Task #407 by frontend-dev (BUG-DL-STALE-STATE-1: auto-clear execution state on workflow ID change)
...
### `client/src/store/SwarmContext.jsx` :: `replaceAgentChatText(nodeId, text)` (NEW — Task #406p2)
- **Purpose:** Replace the entire `agentResults[nodeId].finalText` with a new canonical text string. Used by the `chat_message` WS handler when `isCanonical: true` — the server sends the correctly assembled result text from Claude CLI to fix token-boundary spacing issues in streamed text_delta fragments.
- **Called by:** useSwarm.js::connectWs onmessage (case 'chat_message', when `msg.isCanonical` is true)
- **Calls:** Zustand set
- **Inputs:** nodeId (string — agent node ID), text (string — canonical complete text to replace accumulated fragments)
- **Output:** void (mutates store)
- **Side effects:** Overwrites `agentResults[nodeId].finalText` with the provided text (does NOT append). Sets `viewed: false` and `updatedAt: Date.now()`. Creates the agentResults entry if it does not exist.
- **Last modified:** 2026-04-08 in Task #406 phase 2 by backend-dev

### `client/src/store/SwarmContext.jsx` :: `replaceNodeChatMessages(nodeId, canonicalMsg, predicate)` (NEW — commit 5d359b4, MODIFIED Task #419)
- **Purpose:** Replace ALL matching chat messages for a nodeId with a single canonical message. Used when a canonical result arrives (isCanonical: true) to collapse all text_delta fragments into one clean message, fixing the BUG-CHAT-3 issue where ChatPanel grouping concatenated stale fragments + canonical text, producing duplicated/truncated output.
- **Called by:** useSwarm.js::connectWs onmessage (case 'chat_message', when `msg.isCanonical` is true)
- **Calls:** Zustand set
- **Inputs:** nodeId (string — agent node ID), canonicalMsg (object — the single replacement message with { nodeId, role, text, timestamp, spawnMode }), predicate (function|null — when provided, only messages matching `predicate(m) === true` AND `m.nodeId === nodeId` are removed; when null, all messages for the nodeId are removed)
- **Output:** void (mutates store)
- **Side effects:** Filters out all matching messages from `chatMessages`, remembers the timestamp of the first removed message, inserts the canonical message at the position where the first fragment was (maintains chronological ordering). Falls back to push at end if no insertion point found. Array capped at existing slice limit. **Task #419: returns state unchanged if `canonicalMsg?.text` is falsy — guards against empty canonical destroying existing messages (BUG-CHAT-CLIENT-15).**
- **Complexity note:** The insertion logic finds the index of the first message with a timestamp greater than `firstMatchTimestamp` and splices the canonical message there. This preserves interleaved messages from other nodes in their correct chronological position while replacing all fragments from the target node.
- **Last modified:** 2026-04-09 in Task #419 by frontend-dev (added empty-text guard — BUG-CHAT-CLIENT-15)
...
- **Purpose:** Custom React Flow node for visualizing a single swarm agent. Reads live state from useSwarmStore to render status color, output snippet, handoff count, and cost badge. V5 Wave 3: validation warning badge. POST-V5 FOLLOW-UP 2: drop-preview ghost mode. Task #408 (BUG-DL-COST-VANISH-1): cost badge now uses dual-format read and displays regardless of runtime, not just for structured spawn modes.
- **Called by:** SwarmCanvas.jsx — registered in module-level nodeTypes map as nodeTypes.agent (Task #57.1)
- **Calls:** useSwarmStore (selector: s.agentStates[id], s.agentResults[id]), Handle (from @xyflow/react), Position (from @xyflow/react), stripAnsi (utils/stripAnsi.js), isStructuredSpawnMode (utils/runtimeModes.js)
- **Inputs:** id (string - React Flow node id), data (object - { label: string, systemPrompt?: string, isDropPreview?: boolean }), selected (boolean - React Flow selection state)
- **Output:** JSX - bordered card with status color, agent icon, label, status text, optional validation badge, optional lastOutputSnippet, optional handoffCount badge, optional cost badge (emerald pill showing $X.XX). Preview mode shows "Drop preview" and omits interactive elements.
- **Side effects:** none (read-only store subscription + pure render)
- **Complexity note (Task #408 — BUG-DL-COST-VANISH-1):** Cost is read via `Number(agentState?.totalCost?.costUsd ?? agentState?.totalCostUsd ?? 0)` — a dual-format fallback chain. During live execution, client WS handler accumulates cost under `totalCost.costUsd` (nested object). After completion, `reconcileClosedExecution` may replace agentStates with the server's flat format (`totalCostUsd` at root). The `??` chain handles both. The `isStreamJson` gate that previously hid the cost badge for non-structured runtimes was removed — cost now shows for all runtimes when > 0. Thinking/tool badges still require `isStreamJson` (structured runtimes only).
- **Last modified:** 2026-04-08 in Task #408 by frontend-dev (BUG-DL-COST-VANISH-1: dual-format cost read, removed isStreamJson gate from cost badge)
...
- **Purpose:** Root canvas component for swarm workflow visualization. V5 Wave 3: snapToGrid enabled with 20x20 grid (`snapToGrid` + `snapGrid={[20, 20]}` props on ReactFlow). V5 Wave 2: added NodePalette sidebar (rendered left of ReactFlow), onDragOver/onDrop handlers for palette drag-and-drop (generates nodes via generateNodeId). V5 Wave 1: undo/redo, delete handlers with cascade, context menu, copy/paste/duplicate, keyboard shortcuts, markDirty/onCanvasChange callbacks. POST-V5 FOLLOW-UP 2: Agent palette drags render a live ghost preview node that uses the same snapped flow position as the eventual drop.
- **Called by:** SwarmView.jsx (mounted inside ReactFlowProvider with workflowDef, markDirty, onCanvasChange props)
- **Calls:** useReactFlow (fitView, screenToFlowPosition), useSwarmStore (focusedDepartmentId, setSelectedNode, executionStatus), useNodesState, useEdgesState, useState (dropPreviewNode), useRef (preDragSnapshotRef, updateHistoryTimeoutRef, copyBufferRef, dropPreviewTimeoutRef), useCanvasHistory (pushHistory/undo/redo/canUndo/canRedo), generateNodeId (utils/nodeIdGenerator.js - used in onDrop), useEffect (workflowDef sync, canvas change reporting, keyboard shortcuts, drop-preview cleanup), useMemo (visibleNodes, visibleNodeIds, visibleEdges, renderedNodes, contextMenuActions), useCallback (onConnect, onNodeClick, onPaneClick, onNodeDragStart/Stop, onNodesDelete, onEdgesDelete, handleNodesChange, handleEdgesChange, clearDropPreview, scheduleDropPreviewClear, updateDropPreview, onDragOver, onDrop, context menu handlers, addNodeAtPosition, duplicateNode, deleteNode, deleteEdge, copyNode, pasteNode, handleUpdateNode), NodePalette, ContextMenu, AgentInspector, BreadcrumbBar, InterAgentFeed
...
- **Output:** JSX - flex column: BreadcrumbBar + flex row: NodePalette + ReactFlow canvas (snapToGrid 20x20) + ContextMenu (conditional) + InterAgentFeed (gated on showSidePanels) + AgentInspector (always). `renderedNodes` appends an in-memory `__palette-drop-preview__` node during active Agent palette drags without persisting it into workflowDef.
...
- **Complexity note (V5 Wave 3 — snapToGrid):** `snapToGrid` and `snapGrid={[20, 20]}` props passed to ReactFlow — all node drags snap to 20px grid. No toggle UI yet.
...
- **Complexity note (POST-V5 FOLLOW-UP 2 - live drop preview):** Agent palette drags call `updateDropPreview()` from `onDragOver`. The helper converts screen coordinates with `screenToFlowPosition`, snaps them via `snapPosition()`, and builds a temporary `__palette-drop-preview__` node via `buildCanvasNode({ isDropPreview: true })`. `onDrop()` reuses the same snapped build path, so the ghost and the final node land on the same grid cell. Preview state is never saved; `renderedNodes` appends it only at render time. `clearDropPreview()` runs on pane click, drop, window `dragend`, window `drop`, and after a short timeout so stale ghost nodes disappear even if drag events stop firing.
...
- **Complexity note (V5 Wave 1 — context menu):** Three context handlers (handlePaneContextMenu, handleNodeContextMenu, handleEdgeContextMenu) set contextMenu state with position + type + optional nodeId/edgeId. handleNodeContextMenu and handleEdgeContextMenu call event.stopPropagation() to prevent the pane-level onContextMenu handler from overriding node/edge menus (V5 bugfix 41b9a0e). contextMenuActions useMemo builds action arrays per type. Canvas context: add agent/department/trigger, select all, paste. Node context: edit, duplicate, copy, delete. Edge context: delete.
...
- **Last modified:** 2026-04-07 in POST-V5 FOLLOW-UP 2 by frontend-dev (live drop preview added for Agent palette drags)
...
- **Purpose:** Top-level page shell for the Swarm Orchestrator. V5 Wave 3: added export (handleExport — downloads workflow as JSON Blob), import (handleImport — reads JSON file via hidden file input, validates nodes/edges arrays, POSTs to /api/v1/workflows), duplicate (handleDuplicate — creates copy via apiPost with "(Copy)" suffix), Ctrl+S keyboard shortcut (save via handleSaveRef stable ref), Ctrl+Enter shortcut (run via handleRunRef stable ref), useCanvasValidation integration (validation banner with error/warning counts, Run button disabled when hasValidationErrors), validationErrors state + validationErrorCount + hasValidationErrors derived values, fileInputRef for file import input, importError state. V5 Wave 2: Settings button + WorkflowSettingsModal. V5 Wave 1: Save button, workflow name editing, markDirty/onCanvasChange. Renders toolbar + PromptToFlowBar + saved-workflows selector (with Duplicate/Export/Import buttons) + canvas + HITL drawer + BroadcastBar + PtyExplosion overlay + WorkflowSettingsModal (conditional). Provides ReactFlowProvider boundary.
- **Called by:** App.jsx::MainContent (case 'swarm')
- **Calls:** useSwarmStore (selectors: executionStatus, activeExecutionId, inboxItems, setPaused, setResumed, reset, ptyExplosionNodeId, setPtyExplosionNodeId, workflowDef, setWorkflowDef, selectedRuntimeProvider, setSelectedRuntimeProvider), useAppState, useSwarm, useInbox, useWorkflowList, getPendingCount, useCanvasValidation (hooks/useCanvasValidation.js — V5 Wave 3), apiGet (runtime-capabilities), apiPost (pause/resume, import workflow, duplicate workflow), apiPut (save workflow), sanitizeWorkflow, ReactFlowProvider, SwarmCanvas (with markDirty + onCanvasChange props), PromptToFlowBar, BroadcastBar, PtyExplosion, HitlInbox, WorkflowSettingsModal (conditional on showSettings && workflowDef)
- **Inputs:** none (no props)
- **Output:** JSX — flex-col full-height: toolbar (editable name + Settings + Save + Run/Stop/Pause/Resume/Reset + runtime selector + models + HITL + status) + banners (save error/success, runtime blocker, fallback, validation errors/warnings) + PromptToFlowBar + saved-workflows bar (Load + Refresh + Duplicate + Export + Import buttons) + canvas + HITL drawer + BroadcastBar + PtyExplosion + WorkflowSettingsModal (conditional)
- **Side effects:** Escape key listener. Ctrl+S/Ctrl+Enter listeners (via document.addEventListener). apiPost to pause/resume/import/duplicate. apiPut to save workflow. apiGet for runtime capabilities on mount. Calls store mutations. Calls refreshWorkflows after save/scaffold/import/duplicate. Creates Blob + programmatic <a> click for export download.
- **Complexity note (V5 Wave 3 — validation):** useCanvasValidation called with canvasStateRef.current.nodes/edges. Returns { isValid, errors[] }. Errors synced to validationErrors state via useEffect. validationErrorCount = errors.filter(severity==='error').length. hasValidationErrors gates Run button (disabled + tooltip). Validation banner renders when idle/completed with error/warning badges per item.
- **Complexity note (V5 Wave 3 — keyboard shortcuts):** Ctrl+S and Ctrl+Enter handled in document keydown listener. Uses two ref layers: (1) handleSaveRef/handleRunRef hold state snapshots { isDirty, workflowDef, saving } / { workflowDef, activeProjectId, executing, executionStatus, validationErrors } for gating conditions; (2) handleSaveFnRef/handleRunFnRef hold the actual handleSave/handleRun function references (assigned after function definitions on render). The keydown handler calls `handleSaveFnRef.current?.()` / `handleRunFnRef.current?.()` instead of calling the functions directly, fixing a stale closure bug where Ctrl+S would not trigger save (V5 bugfix 41b9a0e). Ctrl+Enter gated on canRun condition (idle/completed + no validation errors + project selected).
- **Complexity note (V5 Wave 3 — export/import/duplicate):** handleExport: sanitizes workflow, creates JSON Blob, programmatic <a> click download with `.json` extension. handleImport: reads file via FileReader.text(), JSON.parse, validates nodes/edges arrays, POSTs to /api/v1/workflows, resets fileInput.value for re-import. handleDuplicate: sanitizes current workflow, POSTs as new with "(Copy)" name suffix.
...
- **Complexity note (POST-V5 FOLLOW-UP 2 - mount blocker fix):** `reconcileClosedExecution()` awaits `applyExecutionSnapshot()` during hydration/close recovery. Because both are `const ... = useCallback(...)`, declaration order matters: `applyExecutionSnapshot` must be initialized first or the hook throws `ReferenceError: Cannot access 'applyExecutionSnapshot' before initialization` while SwarmView mounts.
- **Last modified:** 2026-04-07 in POST-V5 FOLLOW-UP 2 by frontend-dev (hook-order fix: `applyExecutionSnapshot` moved above `reconcileClosedExecution`)
...
- **Purpose:** Apply a full or partial execution snapshot to useSwarmStore. Handles executionId, status, runtimeBlocker, runtimeProvider, providerStrategy, lastFallback, agentStates, triggerStates, edgeCounters, budget, inboxItems, interAgentFeed. Persists to localStorage when active, clears when terminal. Lazily loads workflowDef via apiGet if not in snapshot and not already in store. Task #408 (BUG-DL-COST-VANISH-1): normalizes server-serialized agentStates by synthesizing nested `totalCost` objects from flat fields (`totalCostUsd`, `totalInputTokens`, `totalOutputTokens`, `totalCachedInputTokens`) so cost data survives reconciliation after completion.
- **Called by:** restorePersistedExecution (hydration path), reconcileClosedExecution (terminal close reconciliation), connectWs onmessage 'execution_status' case
...
- **Complexity note (POST-V5 FOLLOW-UP 2 - hydration unblock):** This callback must be declared before `reconcileClosedExecution()` inside `useSwarm()`. That close-reconciliation path awaits `applyExecutionSnapshot()` during mount/onclose recovery; if the declaration order is reversed, React hits a temporal-dead-zone `ReferenceError` before SwarmView can mount.
- **Complexity note (Task #408 — BUG-DL-COST-VANISH-1):** The normalization loop iterates `snapshot.agentStates` entries and: (1) if server provides flat cost fields but no nested `totalCost`, synthesizes `{ costUsd, inputTokens, outputTokens, cacheReadTokens, cacheWriteTokens }` from `totalCostUsd`/`totalInputTokens`/`totalOutputTokens`/`totalCachedInputTokens`; (2) if server omits cost data entirely but client has accumulated `totalCost`, preserves the client version; (3) preserves client `lastChatSnippet` if server doesn't provide it. This ensures the AgentNode cost badge and ChatMessage cost footer remain visible after execution completes and reconciliation replaces in-memory state with server-serialized state.
- **Last modified:** 2026-04-08 in Task #408 by frontend-dev (BUG-DL-COST-VANISH-1: agentStates normalization for totalCost flat→nested conversion)

### `client/src/hooks/useSwarm.js` :: `reconcileClosedExecution(executionId)` (internal useCallback)
- **Purpose:** Reconcile execution state when the swarm WebSocket closes. First re-fetches `/status`; if the execution is already terminal, applies that snapshot. If the live status lookup fails, falls back to the execution-history endpoint and finally to an inferred terminal status derived from current agentStates.
- **Called by:** connectWs(executionId) -> `ws.onclose`
- **Calls:** apiGet(`/api/v1/swarm/:executionId/status`), applyExecutionSnapshot, clearStoredExecution, useSwarmStore.getState, readStoredExecution, apiGet(`/api/v1/swarm/history/:workflowId/:executionId`), useSwarmStore.setState
- **Inputs:** executionId (string)
- **Output:** Promise\<void\>
- **Side effects:** HTTP GETs to status/history endpoints; may clear localStorage; may mutate activeExecutionId/executionStatus/runtime state in the Zustand store
- **Complexity note:** Close reconciliation is a 3-stage fallback chain: (1) ask the live status endpoint for authoritative state, (2) if that fails, read persisted execution history for the workflow/execution pair, (3) if history is unavailable too, infer `failed`, `completed`, or `stopped` from the current agentStates snapshot. This keeps the UI from being stranded in `running` when the socket disappears near terminal completion.
- **Last modified:** 2026-04-07 in POST-V5 FOLLOW-UP 2 by frontend-dev (kept after `applyExecutionSnapshot` to avoid TDZ crash)
...
### `server/services/SwarmEngine.js` :: `SwarmEngine._normalizeSnippetLine(rawLine)` — MODIFIED Task #479
- **Purpose:** Normalize a single raw PTY output line by stripping leading decoration characters (bullets, box-drawing, Dingbats, checkmarks), collapsing tabs/multi-spaces, and removing trailing prompt/UI artifacts. Used as the first per-line step inside `_isSnippetNoiseLine` and `_buildSemanticSnippet`.
- **Called by:** `SwarmEngine._isSnippetNoiseLine()`, `SwarmEngine._buildSemanticSnippet()` (per-line in filtering loop)
- **Calls:** String.replace (6 regex replacements), String.trim
- **Inputs:** rawLine (string, default '') — a single line from raw PTY output
- **Output:** string — normalized line with leading decoration and trailing noise stripped
- **Side effects:** none (pure function)
- **Complexity note (Task #479 — BUG-BLOCKER-UI-02):** The leading-char strip regex was extended from `\u2720–\u2740` to the full Dingbats block `\u2700–\u27BF` plus explicit checkmarks `\u2713` (✓), `\u2714` (✔), `\u2717` (✗), `\u2718` (✘). This ensures Gemini CLI checkmark-prefixed output lines (common in `provider_unavailable` error flows) are treated as noise rather than content, preventing raw Gemini sign-in lines from leaking into the node card snippet.
- **Last modified:** 2026-04-09 in Task #479 by debugger (BUG-BLOCKER-UI-02: expanded leading-char strip from U+2720–U+2740 to U+2700–U+27BF + explicit U+2713/2714/2717/2718)

### `server/services/SwarmEngine.js` :: `SwarmEngine._handleRuntimeBlocker(executionId, nodeId, blocker)` — MODIFIED Task #479
- **Purpose:** Record a runtime blocker on an agent node — sets `state.runtimeBlocker`, `state.status = 'blocked'`, `state.lastOutputSnippet`, and `state.pinnedDisplaySnippet`. Prunes stale chat messages, resets buffers, and broadcasts a `blocker_hit` WS event. **Task #479 fix:** for `provider_unavailable` blockers the method now bypasses any attempt to sanitize `state.lastOutputSnippet` and calls `_buildRuntimeBlockerDisplaySnippet()` directly to construct the clean blocker message. For all other blocker types, it tries the sanitized existing snippet first and falls back to the clean message only if sanitization yields nothing.
- **Called by:** `SwarmEngine._onStreamJsonResult()` (stream-json error result path), `SwarmEngine._spawnAgentPty` tapFn (PTY runtime — on `__DONE__` with error or on blocker token detection)
- **Calls:** `SwarmEngine._buildRuntimeBlockerDisplaySnippet()`, `SwarmEngine._sanitizeDisplaySnippetText()` (conditionally — skipped for `provider_unavailable`), `SwarmEngine._pruneBlockedChatMessages()`, `SwarmEngine._chatExtractor.resetBuffer()`, `SwarmEngine._broadcastAgentStatus()`
- **Inputs:** executionId (string), nodeId (string), blocker (object — `{ type, message, ... }`) where `type` is one of `provider_unavailable | rate_limited | prompt_rejected | ...`
- **Output:** boolean — false if no matching agent state found; true after applying the blocker
- **Side effects:** mutates `state.runtimeBlocker`, `state.status`, `state.lastOutputSnippet`, `state.pinnedDisplaySnippet`, `state._snippetSourceBuffer`, `state._runtimeScanBuffer`, `state.interventionBuffer`; broadcasts `blocker_hit` WS event; calls ChatExtractor.resetBuffer
- **Complexity note (Task #479 — BUG-BLOCKER-UI-02):** Before this fix, ALL blocker types ran `_sanitizeDisplaySnippetText(state.lastOutputSnippet)` first. For `provider_unavailable` this caused raw Gemini banner noise (sign-in prompts, spinner lines) to appear as the snippet because sanitization left those lines intact. The fix makes `provider_unavailable` unconditionally use the clean synthetic message from `_buildRuntimeBlockerDisplaySnippet()`, bypassing any existing PTY output — which at that point contains only CLI startup noise with no AI work content.
- **Last modified:** 2026-04-09 in Task #479 by debugger (BUG-BLOCKER-UI-02: provider_unavailable path now uses _buildRuntimeBlockerDisplaySnippet directly, skipping sanitize-first logic)
...
| `client/src/hooks/useCanvasValidation.js` | **IMPLEMENTED (V5 Wave 3)** Pre-run validation — checks triage node, orphans, empty prompts, trigger config | SwarmView.jsx (Run button gating), AgentNode.jsx (validation badge) |
...
- **Called by:** SwarmView.jsx::handleSave (before apiPut), SwarmView.jsx::handleExport (V5 Wave 3), SwarmView.jsx::handleDuplicate (V5 Wave 3), server/services/ScaffoldGenerator.js
...
## V5 Wave 3 — New Functions (2026-04-06)

### `client/src/hooks/useCanvasValidation.js` :: `useCanvasValidation(nodes, edges)`
- **Purpose:** Custom hook that validates canvas nodes and edges before workflow execution. Uses useMemo for reactive recomputation. Five validation rules: (1) at least one agent node exists (error), (2) at least one agent has isTriageNode:true (error), (3) agent nodes with empty/whitespace systemPrompt (warning), (4) trigger nodes with missing config — webhook without webhookPath or RSS without rssUrl (error), (5) non-triage agent nodes with zero connected edges (warning). Returns isValid (true when zero errors) and errors array.
- **Called by:** SwarmView.jsx (called as `useCanvasValidation(canvasStateRef.current.nodes, canvasStateRef.current.edges)` — result drives validation banner + Run button gating)
- **Calls:** useMemo (React)
- **Inputs:** nodes (Array — React Flow nodes), edges (Array — React Flow edges)
- **Output:** `{ isValid: boolean, errors: Array<{ nodeId?: string, message: string, severity: 'error'|'warning' }> }`
- **Side effects:** none (pure computation via useMemo)
- **Complexity note:** isValid is derived as `errors.filter(e => e.severity === 'error').length === 0` — warnings do not block execution. Each error includes optional nodeId for future per-node highlighting. Rule 2 (triage check) is skipped when agentNodes.length === 0 to avoid duplicate errors. Rule 5 (disconnected check) exempts triage nodes since they receive external input.
- **Last modified:** 2026-04-06 in V5 Wave 3 by frontend-dev (new file)

### `client/src/views/SwarmView.jsx` :: `handleExport()`
- **Purpose:** Exports current workflow as a downloadable JSON file. Sanitizes the workflow (strips React Flow internals), creates a Blob, generates an object URL, and triggers download via programmatic <a> click.
- **Called by:** SwarmView.jsx (Export button onClick in saved-workflows bar)
- **Calls:** sanitizeWorkflow (utils/sanitizeWorkflow.js), URL.createObjectURL, URL.revokeObjectURL
- **Inputs:** none (reads workflowDef from closure + canvasStateRef.current)
- **Output:** void (triggers browser download)
- **Side effects:** creates and revokes object URL; triggers file download
- **Last modified:** 2026-04-06 in V5 Wave 3 by frontend-dev (new function — FR-V5-38)

### `client/src/views/SwarmView.jsx` :: `handleImport(event)`
- **Purpose:** Imports a workflow from a user-selected JSON file. Reads file text, parses JSON, validates nodes/edges arrays exist, POSTs to /api/v1/workflows to create a new workflow, loads it into the canvas.
- **Called by:** SwarmView.jsx (hidden file input onChange handler)
- **Calls:** file.text(), JSON.parse, apiPost('/api/v1/workflows', ...), setWorkflowDef, setSelectedWorkflowId, refreshWorkflows
- **Inputs:** event (React.ChangeEvent — from file input)
- **Output:** void (async)
- **Side effects:** apiPost creates new workflow on server. Sets workflow state. Resets fileInput.value for re-import. Sets importError on failure.
- **Last modified:** 2026-04-06 in V5 Wave 3 by frontend-dev (new function — FR-V5-39)

### `client/src/views/SwarmView.jsx` :: `handleDuplicate()`
- **Purpose:** Duplicates the current workflow by sanitizing it and POSTing as a new workflow with "(Copy)" appended to the name.
- **Called by:** SwarmView.jsx (Duplicate button onClick in saved-workflows bar)
- **Calls:** sanitizeWorkflow (utils/sanitizeWorkflow.js), apiPost('/api/v1/workflows', ...), setWorkflowDef, setSelectedWorkflowId, refreshWorkflows
- **Inputs:** none (reads workflowDef from closure + canvasStateRef.current)
- **Output:** void (async)
- **Side effects:** apiPost creates new workflow on server. Sets workflow state.
- **Last modified:** 2026-04-06 in V5 Wave 3 by frontend-dev (new function — FR-V5-37)
...
## 2026-04-07 - Debugger-loop hardening notes

### `server/routes/swarm.js` :: terminal execution result resolution
- **Purpose:** Make `/api/v1/swarm/executions/:executionId/results` and `/artifact.md` truthful in the narrow window where an execution is already terminal for the UI but still present in live SwarmEngine memory.
- **Key helpers:**
  - `getHistoryStore(appLocals)` — prefers `app.locals.executionHistoryStore` when present, otherwise falls back to the lazy on-disk singleton.
  - `lookupHistoryExecution(executionId, workflowIdHint, appLocals)` — resolves persisted execution entries without duplicating scan logic.
  - `buildAgentOutputsFromExecution(execution)` — normalizes live `chatMessages` + `agentStates` into the same rich `agentOutputs` shape used by persisted history.
  - `buildLiveExecutionResults(execution, workflowName)` — synthesizes terminal `aggregatedArtifact` and `meta` directly from the live execution record when history is not available yet.
- **Flow:** `lookupExecution(...)` now checks live status first, but if that live status is terminal it tries persisted history before returning live data. If history is still missing, the route serves synthesized terminal results instead of empty artifact content.
- **Impact:** `WorkflowArtifactPanel` and the terminal-state hydration path in `useSwarm.js` no longer hit the old "completed but empty report" gap.

### `client/src/canvas/PromptToFlowBar.jsx` + `client/src/views/SwarmView.jsx` :: stale prompt validation reset
- **Purpose:** Prevent a failed empty-prompt validation from sticking around after the user moves into another valid workflow flow.
- **Flow:** `SwarmView` keeps `promptToFlowResetKey` and increments it on workflow load/generate/import/duplicate/template instantiate/version restore/run. `PromptToFlowBar` listens via `resetSignal` and clears both `promptError` and request `error` on change.
- **Impact:** Prompt-to-Flow validation remains local to the failed prompt attempt instead of leaking into unrelated workflow actions.

### `server/services/ProcessRegistry.js` :: PID validation
...
### `server/services/StreamJsonParser.js` :: `StreamJsonParser._parseResult(obj)` (internal) — MODIFIED Task #406p2
- **Purpose:** Parse the final `type: "result"` NDJSON line (DEC-029). Extracts session ID, cost, duration, full token usage breakdown (input/output/cacheRead/cacheWrite), error detection via is_error flag or subtype "error". **Task #406 phase 2:** now also extracts `resultText` — the canonical complete text from `obj.result` (string only, null otherwise). This field provides correctly assembled text that replaces streamed text_delta accumulation which suffers from token-boundary spacing issues.
- **Called by:** StreamJsonParser.parseLine
- **Calls:** none
- **Inputs:** obj (parsed JSON with type "result")
- **Output:** `{ type: 'result', sessionId, costUsd, durationMs, usage: { input, output, cacheRead, cacheWrite }, isError, errorMessage, resultText }` — resultText is string|null (NEW Task #406p2)
- **Side effects:** none
- **Last modified:** 2026-04-08 in Task #406 phase 2 by backend-dev (added resultText extraction)
