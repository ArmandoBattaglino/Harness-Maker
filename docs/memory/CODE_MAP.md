...
| client/src/canvas/PromptToFlowBar.jsx | default PromptToFlowBar | Natural-language prompt input bar. POSTs to /api/v1/swarm/scaffold, calls onWorkflowGenerated(workflowId, animatedDef) on success. Staggered per-node opacity:0/animation style injection removed (Task #116 — BUG-SWARM-2 fix). @keyframes fadeIn removed from index.css. (Tasks #60, #116) |
...
- **Last modified:** 2026-04-02 in Task #126 by backend-dev (BUG-HANDOFF-1: added case 'handoff_completed' → addFeedEvent; was Task #124 for BUG-SESSION-1 before)
