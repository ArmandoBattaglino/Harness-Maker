...
_Last updated: 2026-03-27 after Task #47.2: swarm.js scaffold stub confirmed; Task #48.2: swarmHandler.js broadcast() + WS event wiring_
...
- docs/ARCHITECTURE.md -- Does not yet include V3 components (WorkflowStore, HandoffParser, SwarmEngine, CircuitBreaker, BudgetTracker, swarm REST routes, swarmHandler WS, BroadcastService, etc.). Will require a major update when V3 is feature-complete (Task #82). SwarmEngine is fully implemented through Task #46.3. CircuitBreaker and BudgetTracker exist as standalone pure-service classes (Task #49). server/routes/swarm.js implements 7 execution control REST endpoints (Task #47.1); scaffold endpoint remains a 501 stub (Task #47.2 — no code change, stub already present). server/ws/swarmHandler.js implements WebSocket channel routing + connection management for /ws/swarm (Task #48.1) and broadcast() named export that fans out JSON events to all open subscriber connections (Task #48.2). swarmEngine.setWsBroadcast(broadcast) called in server/index.js (Task #48.2) — live WS event pipeline is fully wired. Scaffold full implementation (#59) and HITL freeze (#70) are still pending. _onHandoff and _onDone remain stubs until Tasks #62.1–#62.3. server/index.js wires all V3 components.
