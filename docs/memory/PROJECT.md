...
**Implementation status:** v17.6 - task numbering extends through #676. V17.6 PACK DISTRIBUTION & INSTALLATION CLOSED on 2026-04-11 (#662-#669 all COMPLETED/PASS). V17.0-V17.5 are CLOSED. V17.7 PACK FIXTURES & RELEASE GATES is the next planned area (#670-#676). Current V17 distribution baseline includes local bundle export/import, install records, fork-to-draft, workflow dependency rebinding, provenance metadata, and operator distribution controls. Verification: server distribution/store/routes suites 15/15 pass, client distribution/operator suites 4/4 pass, client build 510 modules.
