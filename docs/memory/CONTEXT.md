...
**Focus:** V9.0 STREAM-JSON AGENT MIGRATION CLOSED on 2026-04-08 via #393 PASS. Phase 0 CLOSED, Phase 1 Backend Core CLOSED via #367 PASS, Phase 2 FRONTEND CLOSED via #383 PASS, and Phase 3 INTEGRATION AND POLISH CLOSED via #388 PASS, #390 PASS, #392 PASS, #393 PASS. Debugger-loop follow-up area #394-#396 is also CLOSED. Handoff provider bug fix #397 COMPLETED 2026-04-08 (_ensureAgentPty now passes execution provider mode to _spawnAgent). AUTO mode stream-json routing bug fix #398 COMPLETED 2026-04-08 (_spawnAgent now consults providerStrategy.activeProvider when effectiveProvider is AUTO; 39 PTY tests updated to use explicit providers). Task numbering extends through #398; 394 tasks are currently registered in `docs/TASK_PLAN.md`, 393 are COMPLETE/PASS, 1 is DEFERRED (#236), and 0 are PENDING. PRD v6.0. 478 backend tests pass, client/root build clean.

**IMMEDIATE NEXT STEP:** No registered pending task remains in `docs/TASK_PLAN.md`. 394 tasks registered, 393 COMPLETE/PASS, 1 DEFERRED. The next work should be either a new planned area or non-plan housekeeping such as commit/release prep. The mixed Claude stream-json + Codex PTY path now has deterministic E2E coverage in `server/tests/e2e/stream-json-e2e.test.js`, and the truthful-blocked fallback behavior remains the live-browser baseline for Claude rate-limit/error results. The _spawnAgent dispatcher now correctly routes AUTO mode agents based on providerStrategy.activeProvider, ensuring all Claude agents use stream-json even when no explicit model is configured on workflow nodes.
