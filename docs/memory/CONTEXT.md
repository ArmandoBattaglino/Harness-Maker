...
**Focus:** V9.0 STREAM-JSON AGENT MIGRATION is CLOSED and its debugger-loop follow-ups #394-#399 are all resolved. V9.1 CODEX SDK SWARM INTEGRATION also CLOSED on 2026-04-08 via #405 PASS: `@openai/codex-sdk` is now wired through `CodexSdkAdapter`, SwarmEngine has a `codex-sdk` structured runtime path with truthful stop/reset/resume semantics, and the client treats `codex-sdk` as a first-class structured spawn mode alongside Claude `stream-json`. Task numbering extends through #405; 402 tasks are currently registered in `docs/TASK_PLAN.md`, 401 are COMPLETE/PASS, 1 is DEFERRED (#236), and 0 are PENDING. PRD v6.0. 488 backend tests pass, client build clean at 501 modules.

**IMMEDIATE NEXT STEP:** No registered pending task remains in `docs/TASK_PLAN.md`. 402 tasks registered, 401 COMPLETE/PASS, 1 DEFERRED. The next truthful follow-up is optional: an authenticated live-browser Codex SDK smoke test if we want runtime proof beyond the automated suite. Otherwise the next work should be non-plan housekeeping (commit/release prep) or a brand-new planned area.
