...
- [V5.0-Wave4 COMPLETED 2026-04-06] Execution History, Workflow Templates, and Version History. 5 new files (2 server stores, 3 client panels), 6 modified files. New API endpoints: GET/GET swarm/history, GET workflows/templates, POST templates/:id/instantiate, GET workflows/:id/versions, POST versions/:timestamp/restore. ExecutionHistoryStore persists up to 100 entries per workflow. TemplateStore provides 5 built-in templates. WorkflowStore.update() now saves version snapshots before overwriting. SwarmView toolbar gains History, Templates, and Versions buttons. AgentInspector shows execution timing info. SwarmContext tracks per-agent timestamps.
...
- [V4.0.4 NO-OP 2026-04-06] TASK #203 COMPLETED — BUG-RECOVERY-LABELING-1 already resolved
