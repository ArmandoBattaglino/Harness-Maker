...
| SEC-06 | CSRF header X-Requested-With: ClaudeCodeManager required on mutating requests | PASS | `csrfMiddleware` applied globally before all routes. Checks all POST/PUT/PATCH/DELETE methods. Returns 403 on missing/wrong header. **Exception (Task #234, 2026-04-06):** `/api/v1/triggers/webhooks/` paths are exempt via `CSRF_EXEMPT_PREFIXES` — external webhook callers cannot send the custom header. Protected instead by dedicated rate limiter (10 req/min/IP) and 32KB body cap. | server/middleware/csrf.js:1-35, server/index.js:140 |
